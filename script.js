...
async function getWeather() {
  ...
  const myNumber = "8949432998a191689c44d732a402da4b";
  resultDiv.innerHTML =
    '<div class="loading"></div><p style="margin-top: 10px;">Loading weather data...</p>';
  resultDiv.classList.add("show");

  const url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${myNumber}&units=metric`;
...
function updateBackground(data) {
  ...
  if (
    data.temp >= 30 ||
    data.desc.toLowerCase().includes("sunny") ||
    data.desc.toLowerCase().includes("clear")
  ) {
    document.body.classList.add("sunny-bg");
  } else if (
    data.temp <= 10 ||
    data.desc.toLowerCase().includes("snow") ||
    data.desc.toLowerCase().includes("freezing")
  ) {
    document.body.classList.add("snow-bg");
  } else if (
    data.desc.toLowerCase().includes("rain") ||
    data.desc.toLowerCase().includes("drizzle") ||
    data.desc.toLowerCase().includes("thunderstorm")
  ) {
    document.body.classList.add("rain-bg");
  } else if (
    data.desc.toLowerCase().includes("cloud") ||
    data.desc.toLowerCase().includes("overcast") ||
